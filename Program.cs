...
var path = "";
do{
    Console.WriteLine("Ingrese la direccion de una carpeta de la cual quiere ver los archivos: ");
    path = Console.ReadLine();
}while (path == null);

Console.WriteLine("\nLista de archivos: ");
var archivos = Directory.EnumerateFiles(path);
foreach (string archivo in archivos)
{
    Console.WriteLine("\t" + archivo);
}

using (StreamWriter sw = new StreamWriter(@".\index.csv"))
{
    string[] texto;
    var nombre = "";
    var ext = "";
    ...
    foreach (string archivo in archivos)
    {
        texto = archivo.Split(".");
        if(path != "."){
            nombre = texto[0].Substring(path.Length);
            ext = texto[1];
        }else{
            nombre = texto[1];
            ext = texto[2];
